 ...
 *
 * Last update October 11, 2022 for Hubitat
 */
	...
	if(willQ && canq){
		...
		try{
			...
			if(doI){
				...
				if(skip) msg.m='Command optimization: Skipped execution of'+tstr+' because it would make no change to the device.'+s
...
private Boolean evaluateCondition(Map r9,Map cndtn,String collection,Boolean async){
	...
	if(ffwd(r9) || comparison!=null){
		...
		if(currun(r9) in [iZ,iN9]){
			...
			//if(ro)for(Map value in liMs(ro,sVALUES))updateCache(r9,value,t)
...
private void traverseExpressions(node,Closure closure,prm,parentNode=null){
	...
	if(node.i instanceof List) traverseExpressions(liMs(node,sI),closure,prm,node)
}
		...
		statementTraverser={ Map node,parentNode,Map<String,Boolean>data,Map<String,Integer>lvl ->
			...
			switch(t){
				case sACTION:
					...
					if(node[sK]){
						for(Map k in liMs(node,sK)) traverseStatements(k.p ?: [],statementTraverser,k,data,lvl)
					}
					break
				case sIF:
					if(node[sEI]){
